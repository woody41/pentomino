...
public class PentominoBuilder {
     ...
     */
     ...
     {
       //Check if their dimensions are equal, if not, they can't be equal so return false
       if ((data1.length != data2.length) || (data1[0].length != data2[0].length)) return false;

       //Otherwise, iterate over all elements. If one is not equal to another one, return false since they're not equal.
